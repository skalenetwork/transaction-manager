...
def send_eth(w3: Web3, wallet: BaseWallet, address: ChecksumAddress, amount: int) -> None:
    tx = {'to': address, 'value': amount, 'gas': 8000000, 'gasPrice': w3.eth.gas_price}
    signed_txn = wallet.sign(tx)
    h = w3.eth.send_raw_transaction(signed_txn.raw_transaction)
