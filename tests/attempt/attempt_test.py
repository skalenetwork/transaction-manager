...
def create_attempt(nonce=1, index=2, gas_price=10**9, wait_time=30):
    tid = 'id-aaaa'
    return Attempt(
        tx_id=tid, nonce=nonce, index=index, fee=Fee(gas_price=gas_price), wait_time=wait_time
    )
...
def test_attempt():
    ...
    raw_before_eip_1559 = (
        b'{"gas_price": 1000000000, "index": 2, "nonce": 1, "tx_id": "id-aaaa", "wait_time": 30}'  # noqa
    )
